...
2. Go to `builder/web-console/get-category-plps.js` and also navigate to the desired affiliate home page
...
13. Once we've done the above, we should generate the site map for the app, go ahead and run `npm run gen-site-map`.
...
5. Core => If first time scrape `gen-site-map` to gen the site map
6. Core => PR into develop (dev env)
7. Core => Develop into Main (prod env)
8. Check in vercel/fliik that it's deployed (more info on deployment in the `Deployment` section).
