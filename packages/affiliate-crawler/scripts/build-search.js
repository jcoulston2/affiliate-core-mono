...
import { parseStoreCache, lowerCase } from '../helpers';
import fs from 'fs';
...
async function writePredictiveSearch(searchJson) {
  ...
  const outputFileDir = __dirname + '/../__predictive-search__';
  return new Promise((resolve) =>
    fs.writeFile(`${outputFileDir}/predictive-search.json`, searchJsonString, 'utf8', (err) => {
      if (err) throw err;
      resolve();
    })
  );
