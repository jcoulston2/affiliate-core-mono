/**
 * @Info The page evaluation handles extracting data from a given page through a series of selectors given
 * by and defined in a schema. Note the evaluation function is injected by puppeteer into our chrome instance
 * and therefore has it's own scope in the browser. This menas it has access to all the native DOM methods but
 * means other parts of the code base are not accessible.
 */
export default function pageEvaluations(params) {
  ...
  function extractFromElement(node, methods, isConditional) {
    ...
    const getDataFromText = () => {
      data = node.textContent.trim().replace(/([a-z])([A-Z])/g, '$1 $2');
  ...
  function getProductTags(productName) {
    const productNameSingle = getSingle(productName);
    if (!productNameSingle || typeof productNameSingle !== 'string') return null;
    const specialCharEx = /[!@#$%^&*()_+=[\]{};':"\\|,.<>/?]+/;
    const tagsArray = productNameSingle.trim().toLowerCase().split(' ');
  ...
  function catchInvalidPlpTiles(product, omitDuplicateImgByBase) {
  ...
  function getExtractData({
    ...
    try {
      ...
      const productData = products
        ...
        .filter((product) => catchInvalidPlpTiles(product, omitDuplicateImgByBase));
...
/**
 * @Info Some sites have conditional javaScript rendering whereby HTML is only visible when scrolled to the
 * relevant position in the viewport. When this happens we can scroll the page to allow the relevant data to
 * render. Note this is exported separately and is called in extractor if 'scrollPage' is set to true in the
 * schema.
 */
export async function scrollPageBeforeExtract() {
  const documentHeight = document.body.scrollHeight;
  const revisedScrollHeight = documentHeight < 8000 ? 12000 : documentHeight;
  const scrollBy = 200;
  const scrollIntervals = revisedScrollHeight / scrollBy;
  let nextScroll = 0;
